...
def train(train_x,train_y ,epoch = 10,svm_c = 1):
        ...
        for i in range(len(train_x)):
            ...
            if y * func <= -1:
                ...
                if first:
                    ...
                    test0 = 0
                    for a in range(len(save_x)):
                        test0 += euclidean(save_x[a],w2)
                    test0 /= len(save_x)
                    test0 += svm_c * np.sum(slack)
                    w = w2
                ...
                for b in range(len(save_x)):
                    test1 += euclidean(save_x[b],w2)
                test1 /= len(save_x)
                test1 += svm_c * np.sum(slack)
                if np.dot(w1, save_x[-1]) >= 1-slack[num]:
                    if test1 < test0:
                        w = w2
                        test0 = test1
                    else:
                    ...
                    n += 1
                    slack.append(np.dot(w1, save_x[-1]))
                    # test1 += svm_c*slack.sum()
                    if test1 < test0:
                        w = w2
                        test0 = test1
                    else:
...
# print(type(test_y))
weight = train(train_x,train_y,10,svm_c=3)
...
weight = train(train_x,train_y,10,svm_c=0.001)
y_pred = predic(weight,test_x)
confusion_matrix(y_pred,test_y)
